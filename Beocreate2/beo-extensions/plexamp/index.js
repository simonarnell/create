...
function getPlexampStatus(callback) {
	exec("docker inspect plexamp").on('exit', function(code) {
		if (code == 0) {
...
function setPlexampStatus(enabled, callback) {
	if (enabled) {
		exec("docker-compose -f /opt/beocreate/beo-extensions/plexamp/docker-compose.yaml up -d").on('exit', function(code) {
			if (code == 0) {
				settings.plexampEnabled = true;
		...
		exec("docker-compose -f /opt/beocreate/beo-extensions/plexamp/docker-compose.yaml down").on('exit', function(code) {
			if (code == 0) {
				settings.plexampEnabled = false;
